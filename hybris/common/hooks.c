...
/* helper */
static int hybris_resolve_sysconf(int name)
{
    /* values taken from bionic */
    switch(name) {
...
{
  /* bionic has different values for _SC defines */

  int resolved_name = hybris_resolve_sysconf(name);
  if(-1 != resolved_name) {
    return sysconf(resolved_name);
  }

 ...
 */
typedef void (* gps_location_callback)(GpsLocation* location) FP_ATTRIB;
...
static inline int gps_open(const struct hw_module_t* module, struct gps_device_t** device) {
    return module->methods->open(module, GPS_HARDWARE_MODULE_ID, (struct hw_device_t**)device);
}

static inline int gps_close(struct gps_device_t* device) {

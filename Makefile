...
libEGL.so.1.0: egl/egl.c libhybris_gingerbread.so
	$(CC) -g -shared -Iinclude -o $@ -fPIC -Wl,-soname,libEGL.so.1 $< libhybris_gingerbread.so
	
libEGL.so.1: libEGL.so.1.0
	ln -sf libEGL.so.1.0 libEGL.so.1

test_egl: libEGL.so.1 egl/test.c libhybris_gingerbread.so
	$(CC) -g -Iinclude -o $@ libEGL.so.1 libhybris_gingerbread.so egl/test.c
 
libGLESv2.so.2.0: glesv2/gl2.c libhybris_gingerbread.so
	$(CC) -g -shared -Iinclude -o $@ -fPIC -Wl,-soname,libGLESv2.so.2 $< libhybris_gingerbread.so
...
test_glesv2: libEGL.so.1 libGLESv2.so.2 egl/test.c libhybris_gingerbread.so
	$(CC) -g -Iinclude -o $@ -lm libEGL.so.1 libhybris_gingerbread.so libGLESv2.so.2 glesv2/test.c
